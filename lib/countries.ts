export interface Country {
  ...
  dial: string;
  /** Flag emoji built from regional indicator chars */
  flag: string;
}

/**
 * 240+ countries with ISO codes and dial codes.
 * Sorted by name; India is forced to the top by `popularIso2` below.
 */
...
const POPULAR: string[] = ["IN", "US", "GB", "AE", "CA", "AU", "SG"];

export const COUNTRIES: Country[] = (() => {
  const all: Country[] = RAW.map(([iso2, name, dial]) => ({
    iso2,
    name,
    dial,
    flag: flagEmoji(iso2),
  }));
  const popular = POPULAR.map((iso) => all.find((c) => c.iso2 === iso)).filter(
    (c): c is Country => Boolean(c)
  );
  const rest = all
    .filter((c) => !POPULAR.includes(c.iso2))
...
export function findCountryByDial(
  ...
  const norm = dial.replace(/\D/g, "");
  // longest-prefix match for ambiguous codes (e.g. +1 covers many islands)
  const matches = COUNTRIES.filter((c) => c.dial === norm);
  if (matches.length === 1) return matches[0];
  return COUNTRIES.find((c) => c.dial === norm && c.iso2 !== "US") ?? matches[0];
}
